...
// GET /api/metrics/productivity/trend?days=7&groupType=atd|atp&date=YYYY-MM-DD
  ...
  if (req.query.date) {
    ...
    dates = db.prepare(
      `SELECT DISTINCT snapshot_date FROM uploads WHERE file_type='productivity'
       ...
       ORDER BY snapshot_date DESC LIMIT ?`
    ).all(req.query.date, today, n).map(r => r.snapshot_date).reverse();
  } else {
    dates = lastNDates(n);
  }
...
router.get("/productivity/top", (req, res) => {
  ...
  const rankings = rows
    ...
    .map(r => {
      ...
      return { ...r, quota, progress: r.completed / quota };
    })
    .filter(Boolean)
...
// GET /api/metrics/productivity/groups/trend?days=7&date=YYYY-MM-DD
...
router.get("/productivity/top-by-group/month", (req, res) => {
  ...
  const resolveQuota  = getAdminQuotaResolver(null);
  const excludedNames = getExcludedDesigners();
  const designerMap   = {};

  for (const date of dates) {
    const rows = db.prepare(
      `SELECT group_no, designer_name, job_level, completed, quota
       FROM productivity
       WHERE snapshot_date=? AND on_duty_morning=1
         AND job_level NOT IN (${EXCLUDE_LEVELS.map(() => "?").join(",")})`
    ...
    for (const r of rows) {
      ...
      if (!designerMap[key]) designerMap[key] = { group_no: r.group_no, designer_name: r.designer_name, job_level: r.job_level, total_completed: 0, total_quota: 0 };
      designerMap[key].total_completed += r.completed;
      designerMap[key].total_quota     += quota;
    }
  }

  const byGroup = {};
  for (const d of Object.values(designerMap)) {
    if (!byGroup[d.group_no]) byGroup[d.group_no] = [];
    byGroup[d.group_no].push({
      designer_name: d.designer_name,
      job_level:     d.job_level,
      completed:     d.total_completed,
      quota:         d.total_quota,
      progress:      d.total_quota > 0 ? d.total_completed / d.total_quota : null,
    });
  }

  const groups = Object.keys(byGroup).sort().map(g => ({
    group: g,
    top: byGroup[g].sort((a, b) => b.completed - a.completed || (b.progress || 0) - (a.progress || 0)).slice(0, n),
  }));
