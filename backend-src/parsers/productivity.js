...
// Case type weights: new_case=1, mod=0.6, refinement=0.5, other=1
const WEIGHTS = { new_case: 1, mod: 0.6, refinement: 0.5, other: 1 };
...
// Returns an array of { date, rows, geo } — one entry per completed calendar day found in the file.
// Date comes from each row's complete_date (never from the upload form).
// Today's partial data is excluded automatically.
function parseProductivityFile(buffer) {
  ...
  // Detect case type column from all rows at once (more robust than per-date detection)
  const caseTypeCol = detectCaseTypeColumn(details);
  console.log(`[parser] caseTypeCol=${caseTypeCol} totalDetailRows=${details.length}`);
  if (!caseTypeCol && details[0]) console.log(`[parser] available columns: ${Object.keys(details[0]).join(", ")}`);

  // Exclude today's partial data
  const todayStr = new Date().toISOString().slice(0, 10).replace(/-/g, ""); // "YYYYMMDD"

  // Aggregate per date and designer using complete_date from each row
  const byDate   = {}; // { "2026-05-22": { uid: designerObj } }
  const geoByDate = {}; // { "2026-05-22": { country: count } }

  for (const row of details) {
    const rawDate = String(row.complete_date || "").slice(0, 8);
    if (!/^\d{8}$/.test(rawDate)) continue;
    if (rawDate >= todayStr) continue; // skip today and future
    ...
    byDate[dateISO][uid].completed += WEIGHTS[caseType];
    byDate[dateISO][uid][caseType === "new_case" ? "new_case_count" : caseType + "_count"]++;

    // Geo per date
    const country = String(row.country || "").trim();
    if (country && country !== "NA") {
      if (!geoByDate[dateISO]) geoByDate[dateISO] = {};
      geoByDate[dateISO][country] = (geoByDate[dateISO][country] || 0) + 1;
    }
  }

  const dates = Object.keys(byDate).sort();
  if (!dates.length) throw new Error("No completed data found (all rows are from today or future)");

  console.log(`[parser] dates found: ${dates.join(", ")}`);

  return dates.map(date => {
    const designers = byDate[date];
    const rows = Object.values(designers).map(d => {
    ...
    const geo = Object.entries(geoByDate[date] || {})
      .map(([country, case_count]) => ({ country, case_count }));

    return { date, rows, geo };
  });
}
